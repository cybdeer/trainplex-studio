...
The `WA_NOTIFY_WEBHOOK` URL must be a server we control (or a no-code bridge like Twilio / Whapi / 2chat) that pushes the JSON payload to the **official ops WhatsApp number**. Per house rule, the founder's personal mobile (configured via ``TRAINPLEX_FOUNDER_MOBILE_GUARD``) must never appear as the target - use an official business number.
