...
- **No founder personal number in outbound** (`MEMORY.md → feedback_no_founder_personal_number.md`): Two explicit scan tests (`test_no_founder_mobile_in_weekly_envelope`, `test_no_founder_mobile_in_monthly_envelope`) walk every variant of the configured founder mobile (``TRAINPLEX_FOUNDER_MOBILE_GUARD``) across the weekly + monthly email envelope's subject, body, recipients, and PDF filename. Tests pass.
- **Plain-Hindi bug-fix recap** (`MEMORY.md → feedback_bug_fix_plain_explanation.md`): 3-line founder recap below.
...
- `test_founder_mobile_redacted` — even if the configured founder mobile is planted in a project title, it does not appear in any report (regex scrub).
...
- **No founder personal number in outbound** (`MEMORY.md → feedback_no_founder_personal_number.md`):
  ...
  passes through ``FORBIDDEN_FOUNDER_MOBILE`` (compiled at runtime from ``TRAINPLEX_FOUNDER_MOBILE_GUARD``)
...
| Founder rules honoured | ✅ | All scripts + docs scrub the configured founder mobile (``TRAINPLEX_FOUNDER_MOBILE_GUARD``); INCIDENT_LOG append; no symptom patches |
...
| `feedback_no_founder_personal_number.md` | `feature_flags._scrub_mobile()` on description save; load-test pool uses `+91 90000NNNNN` only; health endpoint test asserts the configured founder mobile (``TRAINPLEX_FOUNDER_MOBILE_GUARD``) is not in body; Monitoring + DR docs explicitly note webhook URLs go to ops channel, never to founder's phone |
...
### Step 10 CI/CD Re-Enable (pending founder env work)

deploy-staging.yml and deploy-prod.yml moved to .github/workflows-disabled/
on 2026-05-15 (commit 0ebca03) to stop email spam from missing secrets.

Re-enable steps (founder action):
1. Provision Hetzner/DO Ubuntu 22.04 VPS @ ~Rs500/month
2. Set DNS staging.trainplex-studio.com -> VPS IP
3. SSH key pair, install Docker
4. Add GitHub Secrets:
   - STAGING_SSH_KEY
   - STAGING_HOST (= staging.trainplex-studio.com)
   - STAGING_USER (= deploy)
   - STAGING_SSH_PORT (= 22)
   - WA_NOTIFY_WEBHOOK (= AiSensy webhook for ops channel only)
5. Move files back: `git mv .github/workflows-disabled/deploy-*.yml .github/workflows/`
6. Push develop -> auto-deploys to staging

Production deploy-prod.yml: same pattern with PROD_* secrets, manual approval gate.

### Step C9 Image Tagging (2026-05-16)

Fork build output retagged to avoid upstream collision risk.
Source image ID: 4db31efe7e48 (built 2026-05-16 01:20 IST, ~1GB).
Tags now pointing at the same image:
  - heartexlabs/label-studio:latest  (kept during transition; retire in Phase 2 P2.6)
  - trainplex-studio:prod              <- referenced by docker-compose.override.yml
  - trainplex-studio:2026-05-16        (date pin)
  - trainplex-studio:086489d           (git SHA pin)
