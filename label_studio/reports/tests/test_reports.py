...
# Literal lives only in ``.env`` (gitignored); resolved at test-time from the
# ``TRAINPLEX_FOUNDER_MOBILE_GUARD`` env var, with a sentinel fallback so
# this source file never embeds the founder's real mobile number.
...
def _resolve_founder_mobile_variants() -> tuple[str, ...]:
...
_FOUNDER_PERSONAL_MOBILE_VARIANTS = _resolve_founder_mobile_variants()
